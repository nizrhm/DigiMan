...
let DigiMan, owner, addr1, addr2;

beforeEach(async function () {
  DigiMan = await ethers.deployContract("DigiMan");
  [owner, addr1, addr2, _] = await ethers.getSigners();
});

describe("Deployment", function () {
  it("Should set the right owner", async () => {
    const marketplaceOwner = await DigiMan.marketplaceOwner();
    expect(marketplaceOwner).to.equal(owner);
  });

  it("Should set the listing fee percentage", async function () {
    const listingFeePercentage = await DigiMan.getListingFeePercent();
...
describe("Creating NFTs", function () {
  it("Should create a new token and listing", async function () {
    ...
    await DigiMan.createToken(tokenURI, price);

    const listing = await DigiMan.getNFTListing(1);
...
describe("Updating Listing Fee", function () {
  it("Should update the listing fee percentage", async function () {
    await DigiMan.updateListingFeePercent(10);
    expect(await DigiMan.getListingFeePercent()).to.equal(10);
  });

  it("Should only allow owner to update the listing fee percentage", async function () {
    await expect(
      DigiMan.connect(addr1).updateListingFeePercent(10)
...
describe("Executing Sales", function () {
  ...
  it("Should execute a sale", async function () {
    const price = ethers.parseEther("1");
    await DigiMan.connect(addr1).executeSale(1, { value: price });
  ...
  it("Should revert if the payment is incorrect", async function () {
    ...
    await expect(
      DigiMan.connect(addr1).executeSale(1, { value: incorrectPrice })
    ).to.be.revertedWith(
...
describe("Retrieving NFTs", function () {
  beforeEach(async function () {
    ...
    await DigiMan.createToken(tokenURI1, price);
    await DigiMan.createToken(tokenURI2, price);
  });

  it("Should retrieve all listed NFTs", async function () {
    const allNFTs = await DigiMan.getAllListedNFTs();
    expect(allNFTs.length).to.equal(2);
  });

  it("Should retrieve my NFTs", async function () {
    const myNFTs = await DigiMan.getMyNFTs();
